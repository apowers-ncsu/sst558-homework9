...
Also, note that I'm renaming things for some clarity with competing models. My MLR models are labeled as `workflow_mlr` for the best of the MLR family of models and `fit_mlr` for the version of this that has trained on the entire training dataset.

```{r}
#renaming my mlr workflow and model and fit
workflow_mlr <- data_workflow3
model_mlr <- data_model
fit_mlr <- workflow_mlr |> fit(data_train)
recipe_mlr <- recipe3
...
Finishing this workflow, now training with the best alpha on the full training set, to complete the LASSO effort. I'm storing the model in `workflow_lasso` and the fit model in `fit_lasso`.

```{r}
workflow_lasso <- workflow_lasso |>
...
Finishing the fit on full training, with best tuning parameters. Model stored in `workflow_regtree` and model fit to training data in `fit_regtree`.

```{r}
fit_regtree <- workflow_regtree |>
  fit(data_train)
```
...
```{r}
fit_regtree |> extract_fit_engine() |> rpart.plot(roundint=FALSE)
```
